...
def get_mosnet_score(wavfile, model):
    # spectrogram
    mag_sgram = mosnet_utils.get_spectrograms(wavfile)
    timestep = mag_sgram.shape[0]
    mag_sgram = np.reshape(mag_sgram, (1, timestep, utils.SGRAM_DIM))

    # make prediction
    mosnet_score, Frame_score = model.predict(mag_sgram, verbose=0, batch_size=1)
...
def main(config):
    ...
    # MOSNet = CNN_BLSTM()
    # model = MOSNet.build()
    # model.load_weights("./pre_trained_mosnet/cnn_blstm.h5")
    ...
    for iters_key in tqdm(converted_wavs_files_dict.keys()):
        ...
        for vc_key in tqdm(converted_wavs_files_dict[iters_key].keys()):
            ...
            for converted_wav in converted_wavs:
                ...
                # Compute MOS of all the wavs

                # Compute MCD of all the wavs
                mcd, penalty, frames = get_metrics_wavs(wav_file_1=original_wavfile_path,
                                                        wav_file_2=converted_wavfile_path,
                                                        n_mfcc=36)
                metrics_vc.append(mcd)
            # Compute mean metrics and add to metrics dict
            metrics_iteration_n_dict[vc_key] = np.mean(np.array(metrics_vc)), np.std(np.array(metrics_vc))
    ...
    with open(csv_file, 'w', newline='') as csvfile:
        ...
        header = ['Iteration', 'Conversion', 'MCD Mean', 'MCD STD']
        csv_writer.writerow(header)

        # Write data to the CSV file
        for iteration, metrics_data in metrics_dict.items():
